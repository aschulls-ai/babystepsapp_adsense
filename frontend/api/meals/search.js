// Vercel API Route - Meals Search (plural endpoint)
export default async function handler(req, res) {
  ...
  const mealRecipes = {
    ...
    'lunch': {
      ...
      '9-12': [
        {
          name: 'Chicken and Vegetable Bites',
          ingredients: ['1/4 lb ground chicken', '1/4 cup finely chopped sweet potato', '2 tbsp breadcrumbs', '1 egg yolk'],
          steps: [
            'Preheat oven to 375°F (190°C)',
            'Steam sweet potato until very soft, mash completely',
            'Mix ground chicken, mashed sweet potato, breadcrumbs, and egg yolk',
            'Form into small finger-sized pieces',
            'Place on lined baking sheet',
            'Bake 15-20 minutes until cooked through (165°F internal temp)',
            'Cool completely before serving'
          ],
          safety: 'Check internal temperature. Cut into appropriate sizes for baby\'s age.',
  ...
  // Parse the query to determine meal type and age appropriateness
  const queryLower = query.toLowerCase();
  let mealType = 'lunch'; // default
  
  if (queryLower.includes('breakfast') || queryLower.includes('morning')) {
    mealType = 'breakfast';
  } else if (queryLower.includes('lunch') || queryLower.includes('midday')) {
    mealType = 'lunch';
  } else if (queryLower.includes('dinner') || queryLower.includes('evening')) {
    mealType = 'dinner';
  } else if (queryLower.includes('snack')) {
    mealType = 'snack';
  } else if (queryLower.includes('chicken') || queryLower.includes('meat')) {
    mealType = 'lunch'; // Default chicken recipes to lunch
  }

  // Determine age group
  const ageMonths = baby_age_months || 9;
  let ageGroup = '9-12';
  ...
  if (recipes.length === 0) {
    // Fallback response for queries without detailed recipes
    const basicResponse = `Here are some ${mealType} ideas for your ${ageMonths} month old baby:\n\n` +
      `**Age-Appropriate Guidelines for ${ageMonths} months:**\n\n` +
      (ageMonths < 6 ? 
  ...
  let response = `# ${mealType.charAt(0).toUpperCase() + mealType.slice(1)} Recipes for ${ageMonths} Month Old\n\n`;
  ...
  response += `## Age-Appropriate Guidelines for ${ageMonths} months:\n\n`;
  response += (ageMonths < 6 ? 
    '• Focus on single-ingredient purees\n• Introduce one new food at a time\n• Watch for allergic reactions\n• No honey, salt, or sugar' :
    ageMonths < 9 ? 
    '• Foods should be soft and mashable\n• Cut foods smaller than baby\'s thumbnail\n• Encourage self-feeding with finger foods\n• Continue breastfeeding or formula' :
    ageMonths < 12 ? 
